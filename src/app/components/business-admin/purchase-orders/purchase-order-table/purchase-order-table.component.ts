...
export class PurchaseOrderTableComponent {
 ...
 constructor(private requestService: RequestService,
  ...
  //     this.sharedService.getData(user.id).subscribe(
  //       (items: any[]) => {
  //         // Process items to filter and split orders based on vendor
  //         const processedItems = items.map((item) => {
  //           // Filter unavailableProducts with status 'Approved'
  //           const approvedProducts = item.unavailableProducts.filter(
  //             (product: any) => product.status === 'Approved'
  //           );
  
  //           // Group products by vendor
  //           const productsGroupedByVendor = approvedProducts.reduce((acc: any, product: any) => {
  //             console.log("product : ",product);
              
  //             const vendorId = product.product.vendor.id;
  //             if (!acc[vendorId]) {
  //               acc[vendorId] = [];
  //             }
  ...
  //           // Create new orders for each vendor group
  //           return Object.keys(productsGroupedByVendor).map((vendorId) => ({
  //             ...item,
  //             unavailableProducts: productsGroupedByVendor[vendorId],
  //           }));
  //         });
  
  //         // Flatten the nested array to make a single list of orders
  //         this.data = processedItems.flat();
  ...
  //           // Group products by vendor and calculate total price for each group
  ...
  //                 acc[vendorId] = {
  //                   products: [],
  //                   totalPrice: 0, // Initialize total price for this vendor
  //                 };
  //               }
  
  //               acc[vendorId].products.push(product);
  //               acc[vendorId].totalPrice += parseFloat(product.product.price); // Add product price
  //               return acc;
  //             },
  //             {}
  //           );
  ...
  //             totalPrice: productsGroupedByVendor[vendorId].totalPrice, // Include total price
  //           }));
  //         });
...
ngOnInit(): void {
  ...
  //     console.warn('No orders selected for merging.'); // Warning if no orders are selected
  //   }
  // }
