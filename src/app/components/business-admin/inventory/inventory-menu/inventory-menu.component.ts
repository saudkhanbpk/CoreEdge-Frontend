...
export class InventoryMenuComponent {
  ...
  get filteredData() {
    return [...this.data, ...this.uploadedData].filter(item => item.MaterialId.includes(this.searchTerm));
  }
  ...
  combineData(): void {
    this.data = [...this.data, ...this.uploadedData]; // Merge uploaded data into main data array
